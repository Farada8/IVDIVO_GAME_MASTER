...
Build/extend an engine only when recurrence, reproducibility, state, coordination or rework justify it. Prefer extending one current engine over creating a duplicate.
...
Radar watches Book/Story, Audio Novel, AI/Models/Tools, Productivity/Software, Market/Distribution and required specialist domains. Research is justified by a material decision, bottleneck, recurring failure, freshness requirement or evidence gap — not novelty alone.

For current providers/APIs/laws/prices/model capabilities/platforms use fresh verification when material. Research ends when a decision is supported, marginal sources stop changing it, a pilot is more informative, evidence is unavailable and recorded, or the task is HOLD/REJECT.
...
**Learning Ledger:** actual observations, experiments, results, reusable learnings.

**Evaluation Matrix:** what evidence can justify saying a book, audio product, workflow or engine is actually better.
...
`CANDIDATE -> DEVELOPMENT CONTRACT -> PILOT/CANARY -> ADVERSARIAL REVIEW -> REGRESSION -> EVALUATION MATRIX -> PROMOTION DECISION -> APPLICATION MAP -> APPLY -> READBACK/TEST -> VERIFIED_CURRENT`.
