#!/usr/bin/env python3
"""Resolve whether an IVDIVO persisted execution state may auto-continue.

This tool does not call providers, spend credits, change canon, or mutate state.
It only reads one JSON state file and emits a fail-closed decision.
"""
...
def _blocker_present(blocker: Any) -> tuple[bool, str]:
    if blocker is None:
        return False, ""
...
def resolve(state: dict[str, Any]) -> dict[str, Any]:
    blocker, blocker_text = _blocker_present(state.get("current_blocker"))
    if blocker:
        return {
    ...
    if isinstance(action, str):
        return {
            ...
            "reason": "next_action is a string; safe/zero_cost/reversible/tool_executable_here must be explicit",
            "next_action": action,
        }
    if not isinstance(action, dict):
        return {
            "decision": "STOP_INVALID_NEXT_ACTION",
            "reason": "next_action must be an object for auto-continuation",
            "next_action": action,
        }
