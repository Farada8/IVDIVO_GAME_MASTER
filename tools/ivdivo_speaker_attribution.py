...
def same_paragraph_prefix(text: str) -> str:
    """Text in the current paragraph immediately after a quote; a leading \n\n is a hard barrier."""
    return text.split("\n\n", 1)[0]


def same_paragraph_suffix(text: str) -> str:
    """Text in the current paragraph immediately before a quote; trailing \n\n yields empty."""
    return text.rsplit("\n\n", 1)[-1]


def last_nonempty_paragraph(text: str) -> str:
    """Nearest non-empty paragraph before a quote, used only for antecedent/standalone rules."""
