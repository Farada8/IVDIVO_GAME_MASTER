...
## Post-pilot rebase
`main` subsequently advanced again. This evidence package is therefore re-created on a clean branch from the fresh later main rather than force-updating the original pilot branch. The underlying pilot evidence remains bound to the original transaction snapshot and is not rewritten as if it happened at the later head.
