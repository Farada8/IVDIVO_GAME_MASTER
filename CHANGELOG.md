...
- `98_CONVERSATION_ARCHIVE/2026-07-20_персонажи-и-архетипы.md` — архив текущего разговора: персонажи, архетипы, статус гипотезы 16/64/1024, конфликт со старой восьмимировой вертикалью.
- `docs/05_WORLDS_AND_CHARACTERS/CHARACTER_ARCHETYPE_INTEGRATION_NOTE_v0.1_DRAFT.md` — тематическая интеграционная заметка: что сохранить как идеи, что не считать каноном, следующий документ `CHARACTER_ARCHETYPE_MATRIX_v0.1_DRAFT`.
- `OPEN_QUESTIONS.md` — открытые вопросы по персонажам, архетипам, синхронизации Drive/GitHub и устаревшим восьмимировым ссылкам.
- `MASTER_INDEX.md` — рабочий Markdown-индекс GitHub, так как файл отсутствовал в репозитории при проверке.

### Changed

- `DECISIONS.md` — DEC-003 помечен как `SUPERSEDED BY DEC-015 / OLD MODEL`; добавлен DEC-015 по `WORLD_ARCHITECTURE_v0.2_APPROVED`: пять вертикальных миров + орбитали параллельных миров.
- `ROADMAP.md` — этап «Восемь миров» заменён на актуальную архитектуру миров; добавлен этап по персонажам, ролям и архетипам.

### Open

- Google Drive `PROJECT_CORE_CONTEXT_2026` всё ещё содержит старую восьмимировую модель и требует отдельной управляемой версии/обновления, потому что `WORLD_ARCHITECTURE_v0.2_APPROVED` уже заменяет её фундаментальный раздел.
- Гипотеза 16/64/1024 сохранена только как банк идей; требуется проектирование `CHARACTER_ARCHETYPE_MATRIX_v0.1_DRAFT` после сверки с core loop.
