...
- `docs/10_KICKSTARTER/DEVELOPMENT_ROADMAP_TO_KICKSTARTER_v0.1_DRAFT.md` (`ROAD-001`) — единый производственный roadmap из 15 этапов: фиксация продукта, механическое ядро, банк 1,000 карт, прототипы 150/300/500 карт, тесты v0.1/v0.2, кампания, blind-test, арт-дирекшн, производственная смета, Kickstarter page, prelaunch и запуск. Для каждого этапа зафиксированы результат, срок, исполнители, документы, критерий готовности, риск и запреты.
...
- `DOCUMENT_REGISTRY.md` — версия повышена до v1.3; добавлена запись `ROAD-001` и связи с механикой, тестированием, продуктом, картами, визуальной системой и Kickstarter.
- `DOCUMENT_REGISTRY.md` — добавлена запись `MECH-002 / CARD-SYSTEM-001` для системы Разломов Реальности.

### Notes

- `ROAD-001` имеет статус `DRAFT / WORKING ROADMAP`; это рабочая производственная последовательность, а не автоматически утверждённое решение основателя.
- Документ вводит обязательные контрольные гейты: Product Lock, Fun Core, Alpha Evidence, System Stability, Campaign Complete, Independent Usability, Commercial Viability и Launch Readiness.
- `MECH-002` имеет статус DRAFT. Это не утверждённый канон и не финальный баланс.
