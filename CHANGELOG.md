...
### Added
- `docs/04_CARDS_AND_COMPONENTS/PORTAL_RISK_DECK_v0.1_DRAFT.md` (CARD-002) — 36 карт `Риск перехода`: 12 мягких, 16 средних, 8 тяжёлых. Колода моделирует космологическую нестабильность перехода: сбой маршрута, потеря ресурса, временное искажение, перенос не туда, след орбитали, повреждение Вихря, расслоение зоны и обратное воздействие мира.

### Changed
- `DOCUMENT_REGISTRY.md` — добавлена строка CARD-002 и обновлено покрытие категории карты/компоненты.

### Status
- Все эффекты, цены предотвращения и последствия CARD-002 имеют статус DRAFT / HYPOTHESIS до плейтеста.
- Новых решений APPROVED не добавлено.
- Google Drive в этом проходе не обновлялся; выполнена только GitHub-запись по запросу пользователя.
