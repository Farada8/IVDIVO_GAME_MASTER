...
class T(unittest.TestCase):
    ...
    def test04_site_summary_missing(self): self.assertEqual(site_context_gate({"site_id":"x","source_role":"EARLY_BRIEF"})["status"],"INSUFFICIENT_SOURCE_NOT_PROJECT_DEFECT")
    ...
    def test10_prompt_protects_unknown(self): self.assertIn("DO_NOT_INVENT:exact_dimensions",prompt_compile({"objective":"mural","protected_unknowns":["exact_dimensions"]})["prompt"])
    ...
    def test20_cost_range(self): self.assertEqual((cost_band({"quantity_basis":10,"unit_cost_low":100,"unit_cost_high":200,"contingency_pct":10})["low"],cost_band({"quantity_basis":10,"unit_cost_low":100,"unit_cost_high":200,"contingency_pct":10})["high"]),(1100.0,2200.0))
    def test21_commission_map(self): self.assertFalse(commission_route({"buyer_class":"hotel","procurement_route":"direct","approval_chain":["owner"],"required_evidence":["site","cost"]})["buyer_intent_claimed"])
    ...
    def test24_offer_ready(self): self.assertEqual(offer_package({"scope":"concept","deliverables":["render"],"assumptions":["site TBD"],"exclusions":["engineering"],"price_basis_state":"RANGE","evidence_state":"E2"})["status"],"OFFER_READY_FOR_REVIEW")
    def test25_redteam_site_fatal(self): self.assertEqual(red_team({"site_authority":False})["status"],"NO_GO")
    def test26_redteam_fake_cost(self): self.assertIn("FAKE_COST_PRECISION",red_team({"site_authority":True,"cost_point_estimate":1000,"maintenance_path":True})["major"])
    def test27_redteam_buyer_overclaim(self): self.assertIn("BUYER_EVIDENCE_MISSING",red_team({"site_authority":True,"buyer_demand_claimed":True,"buyer_evidence":False,"maintenance_path":True})["major"])
    def test28_redteam_pass(self): self.assertEqual(red_team({"site_authority":True,"technical_maturity_claimed":False,"buyer_demand_claimed":False,"maintenance_path":True})["status"],"PASS_WITH_WATCHES")
    def test29_si_capture_only(self): self.assertEqual(self_improvement_observation({"project_id":"p","defect_or_success":"x","root_cause":"y","evidence_ref":"z","repeat_count":1,"proposed_mechanism":"m"})["status"],"CAPTURE_ONLY_NOT_ENGINE_CANDIDATE")
    def test30_si_candidate(self): self.assertEqual(self_improvement_observation({"project_id":"p","defect_or_success":"x","root_cause":"y","evidence_ref":"z","repeat_count":2,"proposed_mechanism":"m"})["status"],"CANDIDATE_FOR_SELF_IMPROVEMENT_REVIEW")
    def test31_si_no_auto_promote(self): self.assertFalse(self_improvement_observation({"project_id":"p","defect_or_success":"x","root_cause":"y","evidence_ref":"z","repeat_count":2,"proposed_mechanism":"m"})["auto_promote"])
