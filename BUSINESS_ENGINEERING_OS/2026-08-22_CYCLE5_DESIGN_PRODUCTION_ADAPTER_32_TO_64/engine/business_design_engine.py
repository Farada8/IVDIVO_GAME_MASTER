...
def design_brief_gate(p):
    ...
    if q["status"]!="ADEQUATE": return q
    if "unknowns" not in p: return {"status":"MISSING_REQUIRED_PROJECT_DATA","missing":["unknowns"],"source_role":p.get("source_role","DESIGN_BRIEF")}
...
def distance_readability(p):
    ...
    return {"status":"READABILITY_PROXY_PASS" if ratio>=0.002 else "READABILITY_PROXY_RISK","proxy_ratio":round(ratio,6),"evidence_ceiling":"ENGINEERING_PROXY_NOT_HUMAN_VIEWING"}
...
def cost_band(p):
    ...
    if q["status"]!="ADEQUATE": return q
    qty=float(p["quantity_basis"]); lo=float(p["unit_cost_low"])*qty; hi=float(p["unit_cost_high"])*qty; cont=float(p["contingency_pct"])/100.0
