...
class DppPreflightTests(unittest.TestCase):
    ...
    def test_expected_supplier_gaps_close(self):
        r = compare_correction(fixture())
        closed = set(r["closed_missing_data_gaps"])
        self.assertIn("dimensions_mm", closed)
        self.assertIn("compliance_document_refs", closed)
        self.assertIn("material_composition", closed)
    ...
    def test_overpacked_registry_is_error(self):
        x = fixture()
        x["corrected"]["registry_metadata"]["full_dpp_payload"] = {"all": "product data"}
        r = validate_record(x["corrected"], "UNKNOWN")
        self.assertEqual(r["disposition"], "INVALID_PREFLIGHT")
        self.assertGreater(r["counts"]["ERROR"], 0)

    def test_populated_field_without_source_is_error(self):
        x = fixture()
        x["corrected"]["candidate_fields"]["dimensions_mm"]["source_ref"] = None
        r = validate_record(x["corrected"], "UNKNOWN")
        self.assertEqual(r["disposition"], "INVALID_PREFLIGHT")

    def test_unknown_source_reference_is_error(self):
        x = fixture()
        x["corrected"]["candidate_fields"]["dimensions_mm"]["source_ref"] = "DOES-NOT-EXIST"
        r = validate_record(x["corrected"], "UNKNOWN")
        self.assertEqual(r["disposition"], "INVALID_PREFLIGHT")

    def test_registry_identifier_mismatch_is_error(self):
        x = fixture()
        x["corrected"]["registry_metadata"]["unique_product_identifier"] = "OTHER"
        r = validate_record(x["corrected"], "UNKNOWN")
        self.assertEqual(r["disposition"], "INVALID_PREFLIGHT")

    def test_out_of_scope_verified_stays_data_map_only(self):
        x = fixture()
        r = validate_record(x["corrected"], "OUT_OF_SCOPE_VERIFIED")
        self.assertEqual(r["disposition"], "TECHNICAL_DATA_MAP_ONLY_OUT_OF_SCOPE_VERIFIED")

    def test_missing_core_identifier_is_gap(self):
        x = fixture()
        x["corrected"]["candidate_fields"]["unique_product_identifier"]["value"] = None
        x["corrected"]["registry_metadata"]["unique_product_identifier"] = None
        r = validate_record(x["corrected"], "UNKNOWN")
        self.assertIn("unique_product_identifier", r["missing_supplier_data"])
