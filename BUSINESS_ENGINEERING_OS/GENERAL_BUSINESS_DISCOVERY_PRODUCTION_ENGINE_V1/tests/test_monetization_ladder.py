...
class MonetizationLadderTests(unittest.TestCase):
    def test_no_artifact_stays_m0(self):
        r = route(MonetizationEvidence("CF-01"))
        self.assertEqual(r["disposition"], "M0_INTERNAL_PROOF_REQUIRED")

    def test_artifact_without_delta_stays_m0(self):
        r = route(MonetizationEvidence("X", technical_artifact=True))
        self.assertEqual(r["disposition"], "M0_INTERNAL_PROOF_REQUIRED")
    ...
    def test_one_paid_diagnostic_only_routes_m2(self):
        r = route(MonetizationEvidence(
            "X", True, True, True, paid_diagnostic_transactions=1
        ))
    ...
    def test_implementation_without_diagnostic_is_invalid(self):
        with self.assertRaises(EvidenceError):
            route(MonetizationEvidence(
                "X", True, True, True, paid_implementation_transactions=1
            ))

    def test_paid_implementation_routes_m3(self):
        r = route(MonetizationEvidence(
            "X", True, True, True,
            paid_diagnostic_transactions=1,
            paid_implementation_transactions=1,
        ))
    ...
    def test_two_recurring_cycles_routes_m4(self):
        r = route(MonetizationEvidence(
            ...
            paid_diagnostic_transactions=1,
            paid_implementation_transactions=1,
            paid_recurring_cycles=2,
            independent_customer_contexts_same_workflow=2,
        ))
    ...
    def test_three_customer_contexts_routes_only_software_candidate(self):
        r = route(MonetizationEvidence(
            "X", True, True, True,
            paid_diagnostic_transactions=3,
            paid_implementation_transactions=3,
            paid_recurring_cycles=3,
            independent_customer_contexts_same_workflow=3,
        ))
    ...
    def test_external_authorization_does_not_promote_evidence(self):
        r = route(MonetizationEvidence(
            "OW-01", True, True, True, external_action_authorized=True
        ))
        self.assertEqual(r["disposition"], "M1_FIXED_SCOPE_DIAGNOSTIC_SPEC_READY_NOT_WTP_PROVEN")
