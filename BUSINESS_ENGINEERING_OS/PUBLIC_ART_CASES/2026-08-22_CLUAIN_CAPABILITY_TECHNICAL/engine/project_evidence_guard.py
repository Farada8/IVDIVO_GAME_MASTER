...
def classify_project_record(record:Dict[str,Any])->Dict[str,Any]:
    ...
    for key in REQUIRED_FIELDS:
        ...
        if value is None or value=="" or value==[] or value=="UNKNOWN": missing.append(key)
    delivery_context=bool(record.get("relevant_delivery_context_proven"))
    if not delivery_context: missing.append("relevant_delivery_context_proven")
    ready=not missing
    if ready: cls="CLASS_A_SUBMISSION_READY"
    elif record.get("title") and record.get("photo_refs") and record.get("context"): cls="CLASS_B_DOCUMENTED_WORK_INCOMPLETE_PROJECT_FIELDS"
    else: cls="CLASS_C_PARTIAL_WORK_RECORD"
...
def bind_image_to_work(*,visual_match_only:bool,documentary_binding:bool)->str:
    if documentary_binding:return "BOUND"
    if visual_match_only:return "HOLD_VISUAL_MATCH_ONLY"
    return "UNBOUND"
def normalize_budget(value:Any,authoritative_not_applicable:bool=False)->Any:
    if authoritative_not_applicable:return "NOT_APPLICABLE_AUTHORITY_BOUND"
    if value in (None,"","UNKNOWN"):return None
    return value
def archive_loss_effect(*,archive_loss_documented:bool,missing_project_field:Any)->Any:
    if archive_loss_documented and missing_project_field in (None,"","UNKNOWN"):return None
