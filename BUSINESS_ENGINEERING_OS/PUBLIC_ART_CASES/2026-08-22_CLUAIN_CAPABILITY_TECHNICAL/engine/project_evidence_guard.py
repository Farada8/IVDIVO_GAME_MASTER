...
def classify_project_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "status": "PASS_SUBMISSION_READY" if ready else "HOLD_INCOMPLETE_PROJECT_RECORD",
        "class": cls,
...
def three_project_gate(records: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "status": "PASS_THREE_PROJECT_EVIDENCE" if ready >= 3 else "HOLD_THREE_PROJECT_EVIDENCE",
        "submission_ready_count": ready,
        "required": 3,
        "records": rows,
    }
...
def archive_loss_effect(*, archive_loss_documented: bool, missing_project_field: Any) -> Any:
    # Archive loss can explain why a field is unavailable; it cannot manufacture it.
