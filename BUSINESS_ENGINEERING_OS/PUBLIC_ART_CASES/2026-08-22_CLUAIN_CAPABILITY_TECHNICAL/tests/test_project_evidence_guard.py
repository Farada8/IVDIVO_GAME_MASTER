...
class ProjectEvidenceTests(unittest.TestCase):
    ...
    def test_three_project_gate_passes_three_complete(self):
        self.assertEqual(
            three_project_gate(
                [full_record("A"), full_record("B"), full_record("C")]
            )["status"],
            "PASS_THREE_PROJECT_EVIDENCE",
        )

    def test_visual_similarity_alone_does_not_bind(self):
        self.assertEqual(
            bind_image_to_work(visual_match_only=True, documentary_binding=False),
            "HOLD_VISUAL_MATCH_ONLY",
        )

    def test_documentary_binding_can_bind_image(self):
        self.assertEqual(
            bind_image_to_work(visual_match_only=True, documentary_binding=True),
            "BOUND",
        )
    ...
    def test_not_applicable_budget_requires_authority(self):
        self.assertEqual(
            normalize_budget(None, authoritative_not_applicable=True),
            "NOT_APPLICABLE_AUTHORITY_BOUND",
        )

    def test_archive_loss_does_not_fill_field(self):
        self.assertIsNone(
            archive_loss_effect(
                archive_loss_documented=True, missing_project_field=None
            )
        )
