...
def split_blocker_state(*, current_pack_complete: bool, supplier_identity_verified: bool, supplier_capability_complete: bool) -> dict:
    authority = "FULL" if current_pack_complete else "INCOMPLETE"
    supplier = "CAPABILITY_COMPLETE" if supplier_capability_complete else ("PARTIAL_IDENTITY_ONLY" if supplier_identity_verified else "UNVERIFIED")
    join_unlocked = current_pack_complete and supplier_capability_complete
    if not current_pack_complete:
        state, next_action = "HOLD_MISSING_AUTHORITY", "ACQUIRE_COMPLETE_CURRENT_OFFICIAL_PACK"
    elif not supplier_capability_complete:
        state, next_action = "HOLD_CAPABILITY_EVIDENCE", "ACQUIRE_VERIFIED_SUPPLIER_CAPABILITY_EVIDENCE"
    else:
        state, next_action = "READY_FOR_REQUIREMENT_JOIN", "RUN_REQUIREMENT_BY_REQUIREMENT_JOIN"
    return {"authority_side": authority, "supplier_side": supplier, "requirement_join_unlocked": join_unlocked, "state": state, "next_action": next_action}
