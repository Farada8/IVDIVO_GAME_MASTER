...
from engine.evidence_delta_guards import HistoricalAnalogUse, historical_analog_admissible, bind_formation_evidence, split_blocker_state, can_assert_bid_decision
...
class Cycle7EvidenceDeltaTests(unittest.TestCase):
    ...
    def test_partial_identity_does_not_unlock_join(self):
        s = split_blocker_state(current_pack_complete=False, supplier_identity_verified=True, supplier_capability_complete=False)
    ...
    def test_full_pack_without_capability_holds_supplier_side(self):
        s = split_blocker_state(current_pack_complete=True, supplier_identity_verified=True, supplier_capability_complete=False)
