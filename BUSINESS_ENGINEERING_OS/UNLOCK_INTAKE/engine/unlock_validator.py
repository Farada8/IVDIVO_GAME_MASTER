...
_RESUME_PATH = _REPO_ROOT / "BUSINESS_ENGINEERING_OS/2026-08-22_BUSINESS_RESUME_GATE_V1/runtime/business_resume_gate.py"
...
resume = _load(_RESUME_PATH, "ivdivo_business_resume_gate")
...
def validate_p225_input(
    *,
    ...
    """Validate a candidate P225 source event without mutating Business authority."""
    if not files:
        return {"status": "HOLD_P225_NO_FILES", "admissible_for_ingest": False, "root_authority_mutated": False}
    ...
    return {
        ...
        "inventory": inventory,
        "root_authority_mutated": False,
        "next_authority_action": "PERSIST_SOURCE_AND_READBACK_BEFORE_CORE_RECONCILIATION",
    }


def validate_p235_input(designation) -> Dict[str, Any]:
    """Validate a case-specific bidder designation candidate without promoting it."""
    generic = bidder.designation_v2_state(designation)
    if not generic.get("explicit"):
        return {"status": generic["status"], "admissible_for_authority_review": False, "root_authority_mutated": False}
    if designation.resource_id != EXPECTED_RESOURCE_ID:
        return {"status": "HOLD_P235_RESOURCE_MISMATCH", "admissible_for_authority_review": False, "root_authority_mutated": False}
    if designation.legal_entity != EXPECTED_LEGAL_ENTITY:
        return {"status": "HOLD_P235_LEGAL_ENTITY_MISMATCH", "admissible_for_authority_review": False, "root_authority_mutated": False}
    if designation.scope != INTERNAL_EVALUATION_SCOPE:
        return {"status": "HOLD_P235_SCOPE_NOT_INTERNAL_ONLY", "admissible_for_authority_review": False, "root_authority_mutated": False}
    ...
    return {
        ...
        "root_authority_mutated": False,
        "next_authority_action": "PERSIST_DECLARATION_AND_READBACK_BEFORE_CORE_RECONCILIATION",
    }


def current_resume_route() -> Dict[str, Any]:
    """Return the canonical current route; candidate validation cannot change it."""
    return resume.route_business_resume(resume.BusinessResumeState())
...
def unlock_readiness(*, p225_result: Optional[Dict[str, Any]] = None, p235_result: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "current_resume_route_before_authority_commit": routed["route"],
