...
def substitution_matrix(alternatives: list[dict], proposed_jobs: set[str]) -> dict:
    ...
    for alternative in alternatives:
        if alternative.get("available") and alternative.get("price_type") in {"FREE", "PUBLIC", "SUBSIDISED", "VENDOR_INCLUDED", "INTERNAL"}:
            covered.update(alternative.get("jobs", []))
    residual = sorted(proposed_jobs - covered)
    return {
        "covered": sorted(covered),
        "residual_unsolved_job": residual,
        "status": "RESIDUAL_JOB_EXISTS" if residual else "NO_RESIDUAL_JOB_HOLD_OR_RESHAPE",
    }
    ...
    for candidate in candidates:
        ...
        for challenger in candidates:
            if candidate is challenger:
                continue
            greater_or_equal = all(float(challenger.get(d, 0)) >= float(candidate.get(d, 0)) for d in dims)
            strictly_greater = any(float(challenger.get(d, 0)) > float(candidate.get(d, 0)) for d in dims)
...
def self_improvement_candidate(failures: list[dict]) -> dict:
    ...
    for failure in failures:
        cases.setdefault(failure["defect"], set()).add(failure.get("case_id"))
    candidates = sorted(defect for defect, case_ids in cases.items() if len({case_id for case_id in case_ids if case_id is not None}) >= 2)
    return {"candidates": candidates, "single_failures_remain_observations": True}


def invariants(lanes: list[dict]) -> dict:
    violations = []
    for lane in lanes:
        if lane.get("public_only") and lane.get("market_grade") not in (None, "E0", "E1", "E2", "E2+"):
            violations.append((lane.get("id"), "PUBLIC_TO_MARKET_LEAK"))
        if lane.get("price") is not None and not lane.get("external_price_signal"):
            violations.append((lane.get("id"), "UNSOURCED_PRICE"))
        if lane.get("proof_upgraded_by_polish"):
            violations.append((lane.get("id"), "POLISH_PROOF_LEAK"))
...
def pa5_e3_gate(evidence: dict) -> str:
    ...
    if any(not evidence.get(key) for key in required):
        return "HOLD_BELOW_PA5_E3"
    if evidence.get("compliment_only"):
        return "HOLD_COMPLIMENT_NOT_BEHAVIOR"
    return "PA5_E3_CANDIDATE_REAL_INTERACTION"


def cycle6_eligibility(lanes: list[dict]) -> dict:
    advanced = [lane["id"] for lane in lanes if int(lane.get("pa_grade_num", 0)) >= 4]
    held = [lane["id"] for lane in lanes if int(lane.get("pa_grade_num", 0)) < 4]
