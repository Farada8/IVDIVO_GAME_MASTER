...
class CrossLaneCycle6Tests(unittest.TestCase):
    ...
    def test_p87_substitution_with_residual(self):
        result = substitution_matrix([{"available": True, "price_type": "FREE", "jobs": ["diagnosis"]}], {"diagnosis", "implementation"})
        self.assertEqual(result["residual_unsolved_job"], ["implementation"])
        self.assertEqual(result["status"], "RESIDUAL_JOB_EXISTS")

    def test_p87_zero_residual_holds(self):
        result = substitution_matrix([{"available": True, "price_type": "PUBLIC", "jobs": ["diagnosis", "implementation"]}], {"diagnosis", "implementation"})
        self.assertEqual(result["residual_unsolved_job"], [])
        self.assertEqual(result["status"], "NO_RESIDUAL_JOB_HOLD_OR_RESHAPE")
    ...
    def test_p90_pareto(self):
        candidates = [
            {"id": "a", "decision_utility": 2, "evidence_accessibility": 2, "kill_power": 2},
            {"id": "b", "decision_utility": 1, "evidence_accessibility": 1, "kill_power": 1},
        ]
        self.assertEqual(pareto_front(candidates), ["a"])

    def test_p91_si_repeat_rule(self):
        result = self_improvement_candidate([
            {"defect": "x", "case_id": "a"},
            {"defect": "x", "case_id": "b"},
            {"defect": "y", "case_id": "a"},
        ])
        self.assertEqual(result["candidates"], ["x"])

    def test_p92_public_to_market_invariant(self):
    ...
    def test_p92_unsourced_price_invariant(self):
        result = invariants([{"id": "a", "public_only": True, "market_grade": "E2+", "price": 100, "external_price_signal": False}])
        self.assertFalse(result["pass"])

    def test_p92_polish_invariant(self):
        result = invariants([{"id": "a", "public_only": True, "market_grade": "E2+", "price": None, "proof_upgraded_by_polish": True}])
        self.assertFalse(result["pass"])
    ...
    def test_p95_pa5_e3(self):
        evidence = {"target_user_class": "SME", "decision_before": "x", "decision_after": "y", "interaction_artifact": "z", "timestamp": "t", "what_changed": "c", "compliment_only": True}
        self.assertEqual(pa5_e3_gate(evidence), "HOLD_COMPLIMENT_NOT_BEHAVIOR")
