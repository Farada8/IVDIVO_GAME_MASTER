...
- Bidder primary evidence delta: PR #253 -> `071d4b37e8d2cef395f9e12bcfb919eefb1158cc` — additive evidence only; no bidder auto-designation or proof-grade promotion.
- Duplicate Cycle9 replay PR #252: **CLOSED / SUPERSEDED / NOT COUNTED AGAIN**.
...
`PUBLIC_REGISTRY_PRESENCE != CURRENT_ACTIVE_STATUS`  
`TAX_REGISTRATION != TAX_CLEARANCE`  
`SELLER_INVOICE != PAYMENT_RECEIPT`  
...
- complete frozen SupplierCapabilityProfile v2;
...
## Current supplier-side evidence — reconciled through PR #253
Evidence state is stronger than identity-only, but still **NOT CASE-BOUND BIDDER CAPABILITY**.

Now proven/observed from redacted primary derivatives:
- `SYNTHESIS-IVDIVO LIMITED` is bound to registration number **796820** by an official CORE-interface screenshot;
- screenshot displays status `Normal`, but capture timestamp is unproven, so this is **not** promoted to authoritative current active status;
- Revenue/ROS material proves tax registration/account state and a timestamped **2026-08-07 historical account state**, but **not Tax Clearance**;
- three seller-issued EWI invoice families across May/June/July 2026 document concrete external-insulation/render scopes, multiple sites and at least two counterparty groups;
- bounded payment search found no independent receipt/remittance artifact, so invoices are **not paid-revenue evidence**;
- one May invoice family has conflicting work-period versions and remains one deduplicated family;
- no actual insurance certificate is recovered;
- none of this designates the company as bidder for resource `8872468`.

Backlog effects from PR #253:
- `P237 = PARTIAL` company evidence;
- `P243 = PARTIAL` tax evidence, no clearance;
- `P244 = HOLD` insurance;
- `P248 = PARTIAL` relevant-work evidence;
- `P235 = HOLD` explicit bidder designation;
- `P250–P255 = BLOCKED` until required authority joins are complete.

Still null/unproven where required: final/current company-status authority where timestamp matters, final activity classification, Tax Clearance, insurance, turnover/working capital, H&S/PSCS competence, target-specific roofing/energy-efficiency capability, named personnel/current capacity, target-fit reference thresholds and procurement eligibility.

PR #253 Drive mirror: `1x_Y-X-Bqd-tbAV1Lkbs2sksXcZAi5-WZ`; private identifiers remain out of public GitHub.
...
### P225–P256 — inherited evidence-acquisition chain, partially advanced by PR #253
Highest-value sequence:
1. P225–P234 acquire/authenticate current official target pack, bind receipt, hash/private-manifest files, prove completeness, reconcile revisions/addenda and build TenderRequirementRegistry;
2. P235–P242 create explicit case-specific BidderDesignationObject and freeze authoritative bidder identity packet; P237 now has partial company evidence but P235 remains HOLD;
3. P243–P249 acquire target-required tax/insurance/finance/H&S/personnel/reference/capacity evidence; P243 and P248 are partial, P244 remains HOLD;
4. P250–P251 build/freeze SupplierCapabilityProfile v2 with UNKNOWN defaults;
5. P252–P254 atomic row join and categorical `MET / UNKNOWN / CURABLE / NONCURABLE / N/A` routing;
6. P255 bounded BID/HOLD/NO-BID candidate only after all preconditions;
7. P256 derive frontier from remaining root uncertainty; otherwise `PROTECT_NO_CHANGE`.
...
The two roots are independent. PR #253 advances some bidder evidence fields but does not remove ROOT_B because case-specific designation and complete capability evidence are still missing. Downstream requirement join/decision needs both roots resolved.

## Stop rules
- Do not repeat P193–P224; they are merged authority.
- Do not infer bidder designation from company identity, invoices or user profile context.
- Do not infer Tax Clearance from tax registration/account evidence.
- Do not infer payment/revenue from seller invoices without independent receipt/remittance evidence.
