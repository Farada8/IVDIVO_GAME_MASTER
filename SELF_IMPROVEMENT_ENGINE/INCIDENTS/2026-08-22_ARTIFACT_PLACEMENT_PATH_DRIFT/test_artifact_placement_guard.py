from artifact_placement_guard import ArtifactPlacementInput, can_transition_to_done_verified, evaluate_artifact_placement
...
def test_sibling_folder_mismatch():
...
def test_missing_start_here_update():
...
def test_failed_start_here_readback():
    result = evaluate_artifact_placement(base(start_here_readback_contains_artifact=False))
    assert "START_HERE_READBACK_MISSING" in result["failures"]


def test_legacy_title_conflict():
    result = evaluate_artifact_placement(base(duplicate_or_legacy_conflict_resolved=False))
    assert "DUPLICATE_OR_LEGACY_CONFLICT" in result["failures"]


def test_cross_store_pointer_required():
    result = evaluate_artifact_placement(base(cross_store_required=True, cross_store_pointer_verified=False))
