...
def evidence_gate(c: EvidenceClaim) -> tuple[bool, tuple[str,...]]:
    req=_REQUIRED.get(c.claim_type,set()); missing=req-set(c.evidence)
...
def causal_model_gate(h: CausalHypothesis) -> str:
    if not h.intervention or not h.intended_effect or not h.feedbacks or not h.guardrails: return "INCOMPLETE_CAUSAL_MODEL"
...
class SequentialLedger:
    def __init__(self, run_ids: Sequence[str]): self.run_ids=list(run_ids); self.results=[]
