...
class T(unittest.TestCase):
    ...
    def test_real_interruption_unqualified_without_readback(self):
        q=qualify_interruption(InterruptionObservation('e',True,('P',),False,False,False)); self.assertFalse(q.qualifies); self.assertIn('PROJECT_SLICE_READBACK_INCOMPLETE',q.reasons)
    def test_synthetic_not_real(self):
        q=qualify_interruption(InterruptionObservation('e',True,('P',),True,True,True,synthetic=True)); self.assertFalse(q.qualifies)
    ...
    def test_evidence_firewall(self):
        ok,missing=evidence_gate(EvidenceClaim('HUMAN_PREFERENCE',frozenset({EvidenceClass.TEST_EXECUTED}))); self.assertFalse(ok); self.assertIn('HUMAN_VALIDATED',missing)
    ...
    def test_ledger_complete(self):
        l=SequentialLedger(['P01','P02']); l.append(RunResult('P01','x','PASS','f')); l.append(RunResult('P02','x','PASS','f')); self.assertTrue(l.complete); self.assertEqual(len(l.digest()),64)
