...
class T(unittest.TestCase):
    def test_authority(self): AuthoritySnapshot('a'*40,'e','VERIFIED_CURRENT','r','l','t').validate()
    ...
    def test_real_unqualified_without_readback(self): self.assertFalse(qualify_interruption(InterruptionObservation('e',True,('P',),False,False,False)).qualifies)
    def test_synthetic_not_real(self): self.assertFalse(qualify_interruption(InterruptionObservation('e',True,('P',),True,True,True,synthetic=True)).qualifies)
    def test_event001_qualifies(self): self.assertTrue(qualify_interruption(InterruptionObservation('e',True,('A','B'),True,True,True)).qualifies)
    def test_one_event_not_promotion(self):
        c=RecoveryEvidenceCounter(); c.add(InterruptionObservation('e',True,('A','B'),True,True,True)); self.assertFalse(c.promotion_ready())
    def test_recovery_threshold(self):
        c=RecoveryEvidenceCounter()
        for i,p in enumerate((('A',),('B',),('A',))): c.add(InterruptionObservation(str(i),True,p,True,True,True))
    ...
    def test_evidence_firewall(self): self.assertFalse(evidence_gate(EvidenceClaim('HUMAN_PREFERENCE',frozenset({EvidenceClass.TEST_EXECUTED})))[0])
