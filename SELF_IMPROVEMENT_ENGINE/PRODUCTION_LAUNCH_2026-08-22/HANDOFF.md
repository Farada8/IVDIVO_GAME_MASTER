...
- baseline PR #386 merge `0fde6da5e5af5a388acb905b79b5b21263a15287`; verified baseline head `5c8e1da06374bf87f9e254898c064212ff415b3e`.
...
- baseline `CONTINUITY -> FINAL` gate was verified by cumulative CI 9/9, but post-merge audit found that PASS was not bound to a content version.
- corrective PR #393 merge `9ca32760f21f394d7ef10ca1fe9334b4c298a75c`; verified corrective head `ba856e93d18ca708b92bd1c43bd1c6b6b2d81883`.
- continuity PASS/FAIL now persists deterministic `content_sha256` across `canon.md`, characters/locations/timeline/plot JSON and every file under `chapters/` and `drafts/` ordered by relative path.
- immediately before FINAL, current content digest is recomputed; missing or mismatching digest rejects FINAL as a stale PASS without mutating stage/version/history.
...
Reason for PL-09 priority: PL-08 is now verified with content-version-bound continuity authorization, so the Wave-2 continuity dependency is satisfied. PL-09 must produce severity-tagged contradiction issues with evidence pairs before automatic continuity PASS can be trusted. PL-07 remains blocked on PL-03; PL-13 remains another ready Wave-2 path but follows PL-09 in the canonical order.
