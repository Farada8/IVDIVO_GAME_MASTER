...
authenticated API calls, and persists only a redacted capability snapshot sealed
by provider_snapshot_contract. No paid synthesis request is issued here.
"""
...
def acquire_snapshot(*, timeout: float = 30.0) -> dict[str, Any]:
    ...
    for model in models_payload:
        if isinstance(model, dict) and isinstance(model.get("model_id"), str) and model.get("model_id"):
            models[model["model_id"]] = _sanitize_model(model)
    ...
    snapshot = {
        ...
        "provenance": {
            ...
            "source": [user_meta, subscription_meta, models_meta, *voice_meta],
        },
        "account": {"fingerprint_sha256": account_fingerprint, **stable_subscription},
        "models": models,
        "voices": voices,
        "volatile": {"captured_at": captured_at, **volatile_subscription},
    }
