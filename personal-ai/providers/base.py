...
class ProviderConfig:
    ...
    def public_dict(self) -> dict[str, Any]:
        """Serializable configuration. Secret values never belong here."""
        return asdict(self)
...
class AIProvider(ABC):
    """Provider-neutral interface used by business/book/agent logic.

    Only `generate` is provider-specific. The other required PL-04 operations are
    normalized transformations that call `generate`, so every adapter obeys the
    same four-method contract without duplicating provider-specific HTTP code.
    """
    ...
    def analyze(self, request: ProviderRequest, *, instruction: str | None = None) -> ProviderResponse:
        ...
        return self._operation(request, operation="analyze", prompt=prompt)

    def classify(self, request: ProviderRequest, *, labels: list[str] | tuple[str, ...]) -> ProviderResponse:
        ...
        return self._operation(request, operation="classify", prompt=prompt)
    ...
    def extract(
        ...
        *,
        ...
        return self._operation(request, operation="extract", prompt=prompt)
