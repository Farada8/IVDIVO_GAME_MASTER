...
class ArtifactPlacementAdapterTest(unittest.TestCase):
    def test_drive_provider_observation_passes_with_raw_ids(self):
        ...
        receipt = receipt_from_drive_observation(intent=intent, artifact_metadata={"id": "FILE", "parent_ids": ["PARENT"]}, start_here_readback_ok=True, start_here_mentions_artifact=True)
    ...
    def test_drive_wrong_parent_fails_closed(self):
        intent = PlacementIntent("drive:ROOT", "drive:PARENT", "drive:START")
        receipt = receipt_from_drive_observation(intent=intent, artifact_metadata={"id": "FILE", "parent_ids": ["WRONG"]}, start_here_readback_ok=True, start_here_mentions_artifact=True)
    ...
    def test_drive_multiple_parents_fail_closed(self):
        intent = PlacementIntent("drive:ROOT", "drive:PARENT", "drive:START")
        receipt = receipt_from_drive_observation(intent=intent, artifact_metadata={"id": "FILE", "parent_ids": ["PARENT", "OTHER"]}, start_here_readback_ok=True, start_here_mentions_artifact=True)
        self.assertEqual(receipt.status, PERSISTED_BUT_MISPLACED)
    ...
    def test_github_missing_file_is_not_persisted(self):
        ...
        receipt = receipt_from_github_observation(intent=intent, repository_full_name="repo", path="root/current/master.md", file_observed=False, current_index_readback_ok=True, current_index_mentions_artifact=True)
