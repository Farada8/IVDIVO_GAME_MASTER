...
При работе над формулировкой принципа "архитектура → философия → базовые практики → отбор → активация" запись была закоммичена как DEC-009 без проверки кросс-ссылок в остальных документах. Выяснилось, что DEC-009 уже был зарезервирован (но не утверждён) в `RES-001` (параметры первой коробки), с зависимыми ссылками в `GAME_CONCEPT_ONE_PAGER`, `KICKSTARTER_PLAN` и `TEST-001`; DEC-010 зарезервирован в `SHMAKOV_ASCENT_PATH`. Исправлено перенумерованием новой записи в **DEC-011**, содержание не менялось. Урок: перед присвоением следующего номера DEC — грепать весь репозиторий на `DEC-0\d\d`, а не только читать DECISIONS.md.
