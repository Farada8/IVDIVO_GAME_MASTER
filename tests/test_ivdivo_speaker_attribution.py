...
def test_pronoun_post_uses_last_nonempty_prior_paragraph():
    ...
    r=sa.classify_and_attribute(xs,{'JANA':['Jana'],'SMITH':['Smith']},{'JANA':'F','SMITH':'M'}); assert r[0].speaker=='JANA'
...
def test_ch01_real_collision_caller_regression_fails_closed():
    xs=[seg('B03_CH01_S0171','NARRATION','\n\nThe caller on Nika’s line said, '),seg('B03_CH01_S0172','DIALOGUE','“The passenger door’s jammed.”'),seg('B03_CH01_S0173','NARRATION','\n\nThe hair on Nika’s forearms lifted.\n\n')]
    assert sa.classify_and_attribute(xs,{'NIKA':['Nika']},{'NIKA':'F'})==[]
