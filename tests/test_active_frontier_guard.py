...
class ActiveFrontierGuardTests(unittest.TestCase):
    ...
    def route(self, relation, discovered="PUBLIC_ART_CLUAIN"):
    ...
    def test_supporting_semantic_relevance_does_not_switch(self):
        self.assertEqual(self.route(SUPPORTING).status, "SUPPORTING_ONLY_KEEP_FRONTIER")
    ...
    def test_conflicting_same_project_claim_fails_closed(self):
        out = self.route(SAME_PROJECT_RELEVANT, discovered="PUBLIC_ART_CLUAIN")
        self.assertEqual(out.status, "HOLD_PROJECT_RELATION_CONFLICT_KEEP_FRONTIER")
        self.assertEqual(out.active_project, self.project)
        self.assertEqual(out.active_next_gate, self.gate)
    ...
    def test_required_cross_lane_dependency_gets_return_token(self):
        ...
        self.assertEqual(out.return_token, f"RETURN_FRONTIER::{self.project}::{self.gate}")

    def test_same_project_required_dependency_needs_no_detour(self):
        out = self.route(REQUIRED_DEPENDENCY, discovered=self.project)
        self.assertEqual(out.status, "USE_IN_CURRENT_FRONTIER")
        self.assertIsNone(out.return_token)

    def test_dependency_without_identified_project_fails_closed(self):
        out = self.route(REQUIRED_DEPENDENCY, discovered=None)
        self.assertEqual(out.status, "HOLD_REQUIRED_DEPENDENCY_WITHOUT_PROJECT")
    ...
    def test_unregistered_relation_is_treated_as_unknown(self):
        out = self.route("INTERESTING_BUT_UNCLASSIFIED")
        self.assertEqual(out.status, "HOLD_AMBIGUOUS_SCOPE_KEEP_FRONTIER")
    ...
    def test_invalid_return_token_fails_closed(self):
        self.assertEqual(resolve_return_token("bad").status, "HOLD_INVALID_RETURN_TOKEN")
