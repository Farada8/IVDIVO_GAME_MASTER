...
def compile_lineage(data):
    ...
    for i,b in enumerate(blocks):
        ...
        if not bid or bid in seen:
            ...
            continue
        seen.add(bid)
        for f in ("scene_id","room_id","required_bed","required_cues","prohibited","evidence_grade"):
            if f not in b:
        ...
        normalized.append(n)
    out={
      "schema_version":"room917.compiled_cue_lineage/1.0",
      "source_schema":data.get("schema_version"),
      "status":"PASS" if not errors else "FAIL",
      "timing_state":"RESOLVED" if normalized and all("start_s" in b for b in normalized) else "SEMANTIC_ONLY",
...
def main():
    ...
    out, errors=compile_lineage(data)
    if out is not None:
        Path(args.out).write_text(json.dumps(out,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    if errors:
        for e in errors: print(e,file=sys.stderr)
        return 2
    print(f"PASS blocks={out['block_count']} timing={out['timing_state']}")
    return 0
