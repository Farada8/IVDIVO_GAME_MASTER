...
RESULT_SCHEMA = "ivdivo.room917_p003b_listener_qc_result/1.3"
...
def source_errors(result: dict[str, Any], automix: dict[str, Any] | None) -> list[str]:
    ...
    if result.get("schema_version") != RESULT_SCHEMA:
        errors.append("LISTENER_RESULT_SCHEMA_NOT_1_3")
...
def pass_b_errors(result: dict[str, Any]) -> list[str]:
    blind = result.get("blind_listen") or {}
    if norm(blind.get("pass_b_targeted_verification")) not in PASS_VALUES:
        return ["PASS_B_NOT_COMPLETE"]
    return []
...
def repair_plan_errors(result: dict[str, Any], plan: dict[str, Any] | None, *, require_clean: bool) -> list[str]:
    ...
    if require_clean:
        if plan.get("status") != "NO_REPAIR_REQUESTED":
            errors.append("REPAIR_PLAN_NOT_CLEAN")
        if repairs:
            errors.append("OPEN_REPAIRS_BLOCK_GO")
        if holds:
            errors.append("OPEN_HOLDS_BLOCK_GO")
    else:
        if not repairs:
...
def pass_c_errors(result: dict[str, Any], receipt: dict[str, Any] | None) -> list[str]:
    ...
    expected = ["stereo_headphones", "ordinary_speakers_or_mono", "mobile_proxy"]
    for key in expected:
...
def evaluate(result: dict[str, Any], plan: dict[str, Any] | None, pass_c: dict[str, Any] | None, automix: dict[str, Any] | None) -> dict[str, Any]:
    ...
    output = {
        "schema_version": "ivdivo.room917_p003b_final_listener_gate/1.0",
        "project": "ROOM917",
    ...
    if requested == "HOLD":
        output["errors"] = sorted(set(base_errors + ["HUMAN_LISTENER_VERDICT_HOLD"]))
        return output
    ...
    if requested == "NO_GO":
        ...
        if not errors:
            output["status"] = "LISTENER_QC_NO_GO"
        return output

    if requested == "REPAIR":
        errors = base_errors + pass_b_errors(result) + repair_plan_errors(result, plan, require_clean=False)
        output["errors"] = sorted(set(errors))
        if not errors:
            output["status"] = "LISTENER_QC_REPAIR"
        return output

    if requested != "GO":
        output["errors"] = sorted(set(base_errors + ["UNKNOWN_FINAL_LISTENER_VERDICT"]))
        return output

    errors = base_errors
    errors += pass_b_errors(result)
    ...
    if believability != "BELIEVE":
        errors.append("GO_REQUIRES_BELIEVABILITY_BELIEVE")
    output["errors"] = sorted(set(errors))
    if not errors:
...
def main() -> int:
    ...
    outcome = evaluate(load(args.result) or {}, load(args.repair_plan), load(args.pass_c_receipt), load(args.automix_eligibility))
