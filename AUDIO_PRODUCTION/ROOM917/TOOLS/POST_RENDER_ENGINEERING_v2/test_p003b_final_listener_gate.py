...
def base_result(mode: str = AUTOMIX) -> dict:
    return {
        ...
        "source_identity": {
            ...
            "allowed_target_identity_mode": ["IMMUTABLE_SOURCE_MASTER", "PROVENANCE_VERIFIED_DERIVED_CANDIDATE", AUTOMIX],
            "observed_sha256": SHA,
...
def run(result: dict, plan: dict | None = None, pb: dict | None = None, pc: dict | None = None, auto: dict | None = None) -> tuple[subprocess.CompletedProcess[str], dict]:
    with tempfile.TemporaryDirectory() as td:
        ...
        if pb is not None:
            bp = root / "pass_b.json"; write(bp, pb); cmd += ["--pass-b-open-receipt", str(bp)]
        if pc is not None:
...
def test_clean_automix_go_passes_listener_gate_not_release() -> None:
    proc, out = run(base_result(), clean_plan(), pass_b(), pass_c(), automix())
...
def test_clean_immutable_source_go_does_not_require_automix_receipt() -> None:
    ...
    proc, out = run(r, clean_plan(), pass_b(), pass_c(), None)
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert out["status"] == "LISTENER_QC_GO"


def test_automix_go_without_eligibility_holds() -> None:
    proc, out = run(base_result(), clean_plan(), pass_b(), pass_c(), None)
    assert proc.returncode != 0
    assert "AUTOMIX_ELIGIBILITY_RECEIPT_REQUIRED" in out["errors"]


def test_go_without_pass_b_receipt_holds() -> None:
    proc, out = run(base_result(), clean_plan(), None, pass_c(), automix())
    assert proc.returncode != 0
    assert "PASS_B_OPEN_RECEIPT_REQUIRED" in out["errors"]


def test_pass_b_target_mismatch_blocks_go() -> None:
    proc, out = run(base_result(), clean_plan(), pass_b(target_sha256="b" * 64), pass_c(), automix())
    assert proc.returncode != 0
    assert "PASS_B_OPEN_RECEIPT_TARGET_SHA_MISMATCH" in out["errors"]


def test_open_fourth_note_gate_blocks_go() -> None:
    r = base_result(); r["open_release_gates"]["scene3_fourth_note_perceptual_lock"] = "OPEN"
    proc, out = run(r, clean_plan(), pass_b(), pass_c(), automix())
    assert proc.returncode != 0
...
def test_pass_c_target_mismatch_blocks_go() -> None:
    ...
    proc, out = run(base_result(), clean_plan(), pass_b(), pc, automix())
    assert proc.returncode != 0
...
def test_repair_verdict_requires_pass_b_and_can_exit_before_pass_c() -> None:
    ...
    proc, out = run(r, plan, pass_b(), None, automix())
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert out["status"] == "LISTENER_QC_REPAIR"
    assert out["release_authority"] is False


def test_repair_without_pass_b_receipt_holds() -> None:
    r = base_result(); r["final_verdict"] = {"believability":"BORDERLINE","release_status":"REPAIR","reason":"One localized defect."}
    plan = clean_plan(); plan["status"] = "REPAIR_PLAN_READY"; plan["repairs"] = [{"patch_id":"P003B_D1"}]
    proc, out = run(r, plan, None, None, automix())
    assert proc.returncode != 0
    assert "PASS_B_OPEN_RECEIPT_REQUIRED" in out["errors"]


def test_no_go_can_exit_after_frozen_pass_a_without_pass_b_or_pass_c() -> None:
    r = base_result(); r["final_verdict"] = {"believability":"DONT_BELIEVE","release_status":"NO_GO","reason":"Actor performance is not credible."}
    proc, out = run(r, None, None, None, automix())
    assert proc.returncode == 0, proc.stdout + proc.stderr
