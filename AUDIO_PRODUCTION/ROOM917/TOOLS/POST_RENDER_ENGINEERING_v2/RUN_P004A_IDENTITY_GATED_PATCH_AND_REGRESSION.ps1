...
if ($AssetContract -eq "") {
  $AssetContract = Join-Path $Room917Root "SOUND_DESIGN\ROOM917_E01_CURRENT_BRANCH_SOUND_ASSET_CONTRACT_v1.json"
}
if ($ReleaseProfile -eq "") {
  $ReleaseProfile = Join-Path $Room917Root "SOUND_DESIGN\ROOM917_E01_RELEASE_TRANSLATION_QC_PROFILE_v1.json"
}
if ($BuildId -eq "") {
  $BuildId = "ROOM917_E01_P004A_" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ")
}
...
python (Join-Path $Here "build_derived_master_provenance.py") --candidate $PatchedMaster --patch-plan $PatchPlan --render-report $RenderReport --regression-report $RegressionReport --build-id $BuildId --out $Provenance
...
Write-Host "P004A selective repair + regression + provenance + machine translation QC complete. P003B human listening remains mandatory before release."
