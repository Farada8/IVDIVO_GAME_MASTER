...
def evidence_class_firewall(*, claimed: str, evidence_class: str) -> dict:
    """Ordinal class only blocks obvious upward laundering; domain relevance still gates use."""
    c, e = claimed.upper(), evidence_class.upper()
    ...
    if EVIDENCE_ORDER[e] < EVIDENCE_ORDER[c]:
        return {
            "status": "BLOCK_EVIDENCE_CLASS_LAUNDERING",
            "allowed": False,
            "required": c,
            "observed": e,
        }
    return {
        "status": "CLASS_CEILING_POSSIBLE_DOMAIN_GATE_STILL_REQUIRED",
        "allowed": True,
        "required": c,
        "observed": e,
    }
...
def promotion_eligibility(candidate: Mapping) -> dict:
    ...
    if blockers:
        return {
            "status": "HOLD_PROMOTION_BLOCKERS",
...
def negative_evidence_retention(*, existing_negative_ids: Iterable[str], proposed_retained_ids: Iterable[str], supersession_map: Mapping[str, str | None]) -> dict:
    ...
    if missing:
        return {
            "status": "BLOCK_NEGATIVE_EVIDENCE_LOSS",
            "allowed": False,
            "missing_unsuperseded": missing,
            "superseded": valid_superseded,
        }
    return {
        "status": "PASS_NEGATIVE_EVIDENCE_RETAINED_OR_SUPERSEDED",
        "allowed": True,
        "missing_unsuperseded": [],
        "superseded": valid_superseded,
    }
