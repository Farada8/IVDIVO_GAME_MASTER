...
class Cycle9ControlTests(unittest.TestCase):
    ...
    def test_machine_cannot_claim_human(self):
        out = c9.evidence_class_firewall(claimed="HUMAN", evidence_class="MACHINE")
        self.assertFalse(out["allowed"])

    def test_source_cannot_claim_provider(self):
        out = c9.evidence_class_firewall(claimed="PROVIDER", evidence_class="SOURCE")
        self.assertFalse(out["allowed"])
    ...
    def test_full_machine_packet_only_review_eligible(self):
        ...
        out = c9.promotion_eligibility(c)
    ...
    def test_required_human_blocks_without_human(self):
        ...
        self.assertIn("human_evidence", c9.promotion_eligibility(c)["blockers"])
    ...
    def test_self_reference_cannot_waive_evidence(self):
        out = c9.self_reference_guard(modifies_self_improvement_gate=True,waives_required_evidence=True,externalized_review=True)
        self.assertFalse(out["allowed"])

    def test_self_reference_needs_externalized_review(self):
        out = c9.self_reference_guard(modifies_self_improvement_gate=True,waives_required_evidence=False,externalized_review=False)
        self.assertFalse(out["allowed"])

    def test_normal_non_self_change_passes(self):
        out = c9.self_reference_guard(modifies_self_improvement_gate=False,waives_required_evidence=False,externalized_review=False)
        self.assertTrue(out["allowed"])
