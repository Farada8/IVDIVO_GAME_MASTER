...
class Cycle8EvidenceContractsTests(unittest.TestCase):
    ...
    def test_02_partial_registry_visibility_fails_closed(self):
        self.assertEqual(
            registry_reservation_view(self.committed, [], complete_visibility=False)["status"],
            "HOLD_PARTIAL_VISIBILITY",
        )

    def test_03_committed_reserved_collision_holds(self):
        self.assertEqual(
            registry_reservation_view(self.committed, ["SI-0015"])["status"],
            "HOLD_COLLISION",
        )
    ...
    def test_08_machine_output_cannot_be_human_signal(self):
        self.assertEqual(
            evidence_claim("MACHINE", "root-1", claim_as="HUMAN_SIGNAL"),
            "REJECT_EVIDENCE_LAUNDERING",
        )
    ...
    def test_16_prompt_ir_omission_fails_parity(self):
        result = prompt_ir_parity(
            {"protected_fact": "A", "prohibition": "B"},
            {"protected_fact": "A"},
            ["protected_fact", "prohibition"],
        )
    ...
    def test_19_machine_perceptual_proxy_cannot_be_human_pass(self):
        self.assertEqual(
            evidence_claim("MACHINE_PROXY", "nmm-root", claim_as="HUMAN_SIGNAL"),
            "REJECT_EVIDENCE_LAUNDERING",
        )
    ...
    def test_21_governor_selects_best_admissible_internal_gate_when_human_blocked(self):
        tasks = [
            ...
            {"id": "SI0015_FRESH_REBASE_REVIEW", "priority": 2, "information_gain": 8, "admissible": True},
            {"id": "D04_HUMAN_SIGNAL", "priority": 1, "information_gain": 10, "admissible": False},
        ]
        self.assertEqual(governor(tasks), "SI0015_FRESH_REBASE_REVIEW")
